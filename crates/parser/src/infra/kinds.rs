...
impl SyntaxKind {
  pub fn from_raw(raw: u16) -> Self {
    assert!(raw <= Self::DelimitedTokenTree as u16);
    // SAFETY: SyntaxKind is repr(u16), and the assertion above ensures that raw
    // is within the contiguous range of declared variants.
    unsafe { std::mem::transmute::<u16, Self>(raw) }
  }
